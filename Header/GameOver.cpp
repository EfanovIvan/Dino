						...
						, m_OptionIndex(0)
{
	...
	m_LableText.setString("Game over");
...
bool GameOver::handleEvent(const sf::Event& event)
{
	...
	{
		if (m_OptionIndex == OptionNames::Repeat)
		{
			requestStackPop();
...
void GameOver::updateOptionText()
{
	if (m_Options.empty())
		return;
